// Ursprünglicher Algorithmus: Wort-für-Wort-Markov mit häufigen Zufallssprüngen.
// Wird abwechselnd mit dem Cut-up-Algorithmus (gedicht.ts) verwendet.
...
export function generateGedichtAlt(contents: string[]): string[] {
  const { bigrams, starters, wordPool } = buildMarkov(contents)

  if (wordPool.length < 5) return ['zu wenig worte']
